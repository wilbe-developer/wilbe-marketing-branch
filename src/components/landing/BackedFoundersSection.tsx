...
export default function BackedFoundersSection({
  ...
  return <div className="mt-8 w-full">
      ...
      <div className="mb-6">
        <h4 className="text-lg font-medium text-gray-500 uppercase tracking-wide">FROM POSTDOCS TO FOUNDERS WITH US: LEADING THE MOVEMENT</h4>
      </div>
...
function FounderCard({
  ...
  return <div className="flex-shrink-0 w-72 sm:w-64 md:w-56 bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow group">
      <div className="relative h-32 sm:h-28 bg-gray-100">
        <img src={founder.image} alt={founder.name} className="absolute inset-0 w-full h-full object-cover" onError={e => {
      ...
      <div className="p-3">
        <div className="flex justify-between items-start mb-1">
          ...
          <div className="flex flex-col gap-1">
            {founder.bsfClass && <span className="bg-blue-100 text-blue-700 text-xs px-1 py-0.5 rounded-full font-medium">
                {founder.bsfClass}
              </span>}
          </div>
        </div>
