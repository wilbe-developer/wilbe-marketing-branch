...
export const parseMarkdown = (text: string): string => {
  ...
  return text
    ...
    // Convert plain URLs to clickable links (with negative lookbehind to avoid URLs already in href attributes)
    .replace(/(^|[^"'\(])(https?:\/\/[^\s<>"']+|www\.[^\s<>"']+)(?![^<]*>|[^<>]*<\/a>)/g, '$1<a href="$2" target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:text-blue-800 underline">$2</a>')
